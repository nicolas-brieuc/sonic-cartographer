...
async function validateToken(c) {
...
function handleJsonError(error, c) {
  if (error instanceof SyntaxError) {
    return c.json({ error: "Invalid JSON" }, 400);
  }
  return c.json({ error: "Internal server error" }, 500);
}
async function checkRateLimit(userId, endpoint, _env) {
...
var api_gateway_default = class extends Service {
  async fetch(request) {
    const honoCtx = {
...
// <stdin>
var stdin_default = api_gateway_default;
export {
  cors,
