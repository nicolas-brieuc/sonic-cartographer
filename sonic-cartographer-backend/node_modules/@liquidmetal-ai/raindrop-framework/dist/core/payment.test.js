...
describe('payment', () => {
    ...
    beforeEach(() => {
        ...
        mockPaymentsBinding = {
            ...
            get: vi.fn().mockReturnValue(mockPaymentsActor),
        };
        // Mock logger
        mockLogger = {
            info: vi.fn(),
    ...
    describe('Authentication & Environment Checks', () => {
        it('should return false when no JWT in env (unauthenticated)', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('Payment verification called without authenticated JWT');
        });
        it('should return false when no payments binding in env', async () => {
            ...
            const result = await verifyPayment(request, env);
            expect(result).toBe(false);
            expect(mockLogger.error).toHaveBeenCalledWith('payments binding not found in environment for paid service');
        });
    ...
    describe('Metadata Healing', () => {
        it('should heal WorkOS metadata in background when subscription found', async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Updating WorkOS metadata in background to heal stale JWT', {
                userId: 'user_123',
                productId: 'prod_123',
                subscriptionId: 'sub_789',
            });
            ...
            expect(mockPaymentsActor.updateWorkOSMetadata).toHaveBeenCalledWith('user_123', 'prod_123', 'cus_456', 'active');
        });
