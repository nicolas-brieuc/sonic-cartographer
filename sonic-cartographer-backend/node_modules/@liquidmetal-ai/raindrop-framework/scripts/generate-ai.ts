...
function generateUnifiedTypeMappings(
  ...
  const getCapabilitiesFromCatalog = (modelName: string): string[] => {
    ...
    if (!capabilityMatch) return ['chat'];
    // Parse the array of capabilities
    return capabilityMatch[1].split(',').map(c => c.trim().replace(/'/g, ''));
  };

  const hasCapability = (modelName: string, capability: string): boolean => {
    return getCapabilitiesFromCatalog(modelName).includes(capability);
  };

  // Generate lists of models by category using catalog capabilities
  const externalChatModels = models
    .filter(m => m.tags.includes('external') && hasCapability(getCleanModelName(m), 'chat'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const embeddingModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'embeddings'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const audioModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'audio'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const ttsModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'tts'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const imageGenerationModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'image-generation'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const textClassificationModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'text-classification'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const imageClassificationModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'image-classification'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const translationModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'translation'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const summarizationModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'summarization'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');

  const visionModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'vision'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');
...
export type AiModelInputMap = {
  [K in AvailableModel]: 
    ...
    K extends ${embeddingModels || 'never'} ? ({ input: string | string[]; model?: string; encoding_format?: 'float' | 'base64'; dimensions?: number } | { prompt: string }) :
...
export type AiModelOutputMap = {
  [K in AvailableModel]: 
    ...
    K extends ${embeddingModels || 'never'} ? (OpenAIEmbeddingOutput & { embeddings?: Array<{ embedding: number[] }> }) :
...
function generateModelCatalog(models: ApiModel[]): string {
  ...
  // Sort models to prioritize external over cloudflare (external models come first for deduplication)
  const sortedModels = [...models].sort((a, b) => {
    ...
    // External models come first (higher priority)
  ...
  sortedModels.forEach(model => {
    ...
    const primaryCapability = taskToCapability[model.task?.name as keyof typeof taskToCapability] || 'chat';
    capabilities.push(primaryCapability);
    ...
    // Handle multimodal models that support both chat and vision
    if (model.name.includes('llava') || model.name.includes('vision') || model.name === 'llama-4-maverick-17b') {
    ...
    // Build catalog entry with optional rate limits
    let catalogEntry = `  {
    name: '${cleanName}',
    capability: [${capabilities.map(c => `'${c}'`).join(', ')}],
    provider: '${provider}',
    ...
    if (model.rateLimits) {
      catalogEntry += `,
    rateLimits: {`;
      if (model.rateLimits.requestsPerSecond !== undefined) {
        catalogEntry += `
      requestsPerSecond: ${model.rateLimits.requestsPerSecond}`;
        if (model.rateLimits.tokensPerMinute !== undefined) {
          catalogEntry += ',';
        }
      }
      if (model.rateLimits.tokensPerMinute !== undefined) {
...
async function loadModelRouterConfig(): Promise<{ models: ApiModel[], schemas: Record<string, ModelSchema> }> {
  try {
    ...
    uniqueModels.forEach(modelName => {
      // Skip Baseten test models and specific excluded models
      if (modelName === 'gpt-oss-120b-test') {
        console.log(`⚠ Skipping Baseten test model: ${modelName}`);
        return;
      }
      if (modelName === 'qwen-3-coder-480b') {
      ...
      if (modelName.includes('whisper')) {
        ...
        capability = 'chat'; // Primary capability - generateModelCatalog will add 'vision' for multimodal
        taskName = 'Text Generation';
      }

      // Set model-specific rate limits
      let rateLimits: { requestsPerSecond?: number; tokensPerMinute?: number } | undefined;
      if (modelName === 'glm-4.6-ludicrous') {
        rateLimits = {
          requestsPerSecond: 20,  // Higher than default 10 req/s
          tokensPerMinute: 2000000  // 2x the default 1M tokens/min
        };
      }
      ...
      if (capability === 'audio') {
        ...
        // Chat/completion models with full OpenAI-compatible interface
        schemas[modelName] = {
          success: true,
          result: {
            input: {
...
async function generateAiTypeScript(): Promise<void> {
  try {
    ...
    const catalogEntries = models.map(model => {
      ...
      return `name: '${cleanName}', capability: [${capabilities.map(c => `'${c}'`).join(', ')}]`;
    });
